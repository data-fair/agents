/**
 * The Agent SDK and MCP SDK are optional peers of the bridge (package.json).
 * Without them, df-agents-bridge must fail with an actionable message naming
 * the install command, not node's raw ERR_MODULE_NOT_FOUND. This is tested by
 * asserting the message constant directly, not by uninstalling the SDK.
 */
import { test } from 'playwright/test'
import assert from 'node:assert/strict'
import { MISSING_SDK_MESSAGE, isMissingSdkError } from '../../../lib-sim/bridge/missing-sdk.ts'

test.describe('bridge missing-sdk message', () => {
