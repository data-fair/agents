  ...
  <template v-else>
    ...
    >
      <span class="text-caption font-weight-medium">
        {{ summary.requestCount }} {{ t('requests') }} · {{ formatDuration(summary.totalDurationMs) }} · {{ formatTokens(summary.inputTokens) }} {{ t('in') }}<template v-if="summary.cachedInputTokens != null"> ({{ formatTokens(summary.cachedInputTokens) }} {{ t('cached') }})</template> · {{ formatTokens(summary.outputTokens) }} {{ t('out') }}<template v-if="summary.totalCost != null"> · {{ formatCost(summary.totalCost) }}</template>
      </span>
      <v-spacer />
      <v-chip
