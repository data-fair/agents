...
**Model IDs are roles, not model names.** The client requests `assistant`, `tools`, `summarizer`, or `evaluator` — the server resolves which provider/model to use by walking the [layered configuration and role fallback chain](./configuration.md). The `moderator` role exists in settings but is internal (used by the gateway's [moderation guard](./moderation.md), not client-callable).
