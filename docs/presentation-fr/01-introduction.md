...
data-fair/agents est un service d'assistant conversationnel embarqué dans les applications de la plateforme data-fair, aussi bien les interfaces d'administration que les portails de données publics. Il donne accès à plusieurs fournisseurs de grands modèles de langage, comme OpenAI, Anthropic, Google, Mistral, Scaleway ou tout serveur compatible avec l'API OpenAI, à travers une passerelle unique, sans que l'application hôte ait à gérer elle-même ces intégrations.
