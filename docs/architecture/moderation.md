...
**Self-test.** An admin can opt a single request into the gate with the `x-moderation-self-test` header (`isSelfTestModeration`): the verdict applies normally but the run accrues no strikes and writes no event, so stats reflect only real untrusted traffic. This powers the probe on the activity page.
